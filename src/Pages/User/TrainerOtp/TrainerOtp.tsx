...
import { TutorotpResend, verifyTutorOtp } from "@/Api/trainer";
...
const TrainerOtp: FC = () => {
    ...
    const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await verifyTutorOtp(otp)

            console.log("Verification response:", response?.data);
            setOtp("");
            
            if (response?.data.success) {
                toast.success('Registration successfull,wait for the admin to verify this account')
                navigate('/home');
            }else{
    ...
    const handleResendOtp = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        setSeconds(30)
        console.log(email)
        const res = await TutorotpResend(email)
        if (res?.data.success) {
